...
void ParticleComponent::Initialize(Object* owner, ID3D12Device* device, UINT particle_count, eShape shape, Material* material)
{
	...
	for (int i = 0; i < capacity_; ++i)
	{
		...
		else if (shape_ == Rect)
		{
			...
			particles_.emplace_back(Particle(color, position, XMFLOAT2(10.0f, 20.0f))); // sphere 텍스쳐 사이즈
		...
		else if (shape_ == UpCone)
		{
			...
			particles_.emplace_back(Particle(color_, position, XMFLOAT2(0.1f, 0.1f))); // cone 텍스쳐 사이즈
			particle_data_.emplace_back(ParticleData(direction, 2.0f, 0.1f, 0.8f));	//cone
		}
		else
		{
			assert(false);
